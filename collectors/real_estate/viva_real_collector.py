"""
VivaRealCollector — coleta anúncios de imóveis do Viva Real via JSON API.

Usa a Glue API do Viva Real (mesma infraestrutura do Zap Imóveis — Grupo OLX).
Estratégia idêntica ao ZapImoveisCollector, apenas com portal=VIVAREAL.
...
_GLUE_API = "https://glue-api.vivareal.com.br/v2/listings"
_PORTAL = "VIVAREAL"
_PAGE_SIZE = 24
...
def _extract_listing(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if listing_url and not listing_url.startswith("http"):
        listing_url = f"https://www.vivareal.com.br{listing_url}"
...
class VivaRealCollector(BaseCollector):
    metadata = CollectorMetadata(
        name="real_estate.viva_real",
        domain=CollectorDomain.real_estate,
        source="viva_real",
        description=(
            "Coleta anúncios de imóveis do Viva Real via Glue API. "
            ...
            for business in _BUSINESS_TYPES:
                ...
                for page in range(max_pages):
                    ...
                    try:
                        ...
                        logger.warning(
                            "VivaReal fetch failed",
                            extra={"business": business, "page": page, "error": str(exc)},
                        )
                        break
                    ...
                    for raw in listings_raw:
                        try:
                            ...
                            items.append(
                                CollectedItem(
                                    external_id=f"VIVAREAL-{listing_id}",
                            ...
                            logger.debug("VivaReal parse error", extra={"error": str(exc)})

                    logger.info(
                        "VivaReal page collected",
                        extra={"business": business, "page": page, "count": len(listings_raw)},
                    )
                    await asyncio.sleep(1.5)

        logger.info("VivaReal collection complete", extra={"total_items": len(items)})
        return items
